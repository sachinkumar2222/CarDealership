...
fun ArticleScreen(
    ...
    val uiState by viewModel.uiState.collectAsState()
        ...
        Column(
            ...
            Text("Manage your article content from here", style = MaterialTheme.typography.bodyMedium, color = Color.Gray)
            ...
            Row(
                ...
                Button(onClick = { navController.navigate(HomeRoutes.AddEditArticle()) }) {
                    Icon(Icons.Default.Add, contentDescription = "Add New Article", modifier = Modifier.size(18.dp))
                    Spacer(modifier = Modifier.width(8.dp))
        ...
        LazyColumn {
            item {
                Row(
                    ...
                    TableCell(text = "Sr No.", weight = 0.1f, fontWeight = FontWeight.Bold)
                    ...
                    TableCell(text = "Status", weight = 0.15f, fontWeight = FontWeight.Bold)
                    TableCell(text = "Actions", weight = 0.2f, fontWeight = FontWeight.Bold)
                }
                Divider()
            }
            ...
            itemsIndexed(articles) { index, article ->
                Row(
                    ...
                    TableCell(text = (index + 1).toString(), weight = 0.1f)
                    TableCell(text = article.title, weight = 0.4f)
                    TableCell(text = article.domainName, weight = 0.25f)
                    TableCell(text = article.status, weight = 0.15f)
                    Row(
                        modifier = Modifier.weight(0.2f),
                        horizontalArrangement = Arrangement.spacedBy(8.dp)
                    ) {
                        IconButton(onClick = { onEditClick(article) }, modifier = Modifier.size(20.dp)) {
                            Icon(Icons.Default.Edit, contentDescription = "Edit", tint = Color.Gray)
                        }
                        IconButton(onClick = { onDeleteClick(article) }, modifier = Modifier.size(20.dp)) {
